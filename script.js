...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (signupForm) {
        signupForm.addEventListener('submit', function (e) {
            ...
            (async function () {
                ...
                // Persist login in localStorage so session survives browser close
                try { localStorage.setItem('userName', name); localStorage.setItem('userEmail', normalizedEmail); localStorage.setItem('userRole', role); localStorage.setItem('userLoggedIn', 'true'); } catch (e) { }
                sessionStorage.setItem('userLoggedIn', 'true');
    ...
    // If on index/splash page, handle routing
    ...
    if (isIndexPage) {
        ...
        if (!isLoggedIn) {
            ...
            // Already logged in, visiting index directly — auto go to dashboard
            console.log('[auth] already logged in — auto forwarding to dashboard');
            setTimeout(() => { window.location.href = 'dashboard.html'; }, 150);
        }
    }
